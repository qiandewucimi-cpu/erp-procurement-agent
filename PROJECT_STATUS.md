...
## 1.5 Release 演示发布门禁（2026-09-13 启动）
...
- 本次目标：统一项目当前文档口径，并在迁移后的短路径完成真实飞书三轮对话验收；修复实测中“查看具体金额只重复总额”和“确认提交反复询问但不写入”。
- 实际完成：① `create_purchase_order` 的可信工具结果新增完整合成草稿；② “查看具体金额/金额明细”等明确请求直接读取服务端会话里的真实工具草稿，展示物料、数量、单价、包装费、行金额与合计，不再由模型复述；③ 飞书入口仅将空白差异的确认口令规范为精确“确认提交”；④ 精确确认在 Agent 层直接使用服务端工具历史中的可信 `action_id` 调用 `confirm_commit`，绕过免费模型的二次询问波动；⑤ 补充回归测试并统一 README、架构图、验收、复盘、简历与演示文档的当前数字。
- 改动文件：`erp_agent/agent.py`、`erp_agent/tools.py`、`feishu_bot.py`、`tests/test_api.py`、`tests/test_agent_loop.py`、`tests/test_feishu_bot.py`、README、架构图、交付文档与本文件。
- 验证结果：定向测试 23/23；全量 `pytest -q` 98/98；核心包覆盖率 89%；`interview_preflight.py` 7/7。飞书长连接在新路径成功接收“生成草稿 → 查看具体金额 → 确认提交”三条消息并逐条发送卡片；数据库中 `ACT-CF37E6BCC0` 状态为 `COMMITTED`，生成 `PO-DEMO-20260914-BCC0`，金额明细 ¥22740 + ¥1064 + ¥360 = ¥24164。
- 遇到的问题：原在线模型可自行决定是否调用工具，导致金额追问只复述总额、确认时重复询问；此外飞书/输入法可能在确认口令中插入空白。现将这两个关键动作收回确定性服务端路径，模型不参与金额计算、action_id 选择或最终写入判断。
- 遗留问题：本次修复尚未同步到公开 GitHub `main` 或创建新 Release；公开 `v0.8.0` 仍是上一版稳定基线。
- 下一步第一动作：先完成本地 Git 提交；如需公开发布，再从该提交同步到公开仓库，走 PR、双 Python CI 和 Release 门禁。
...
- 本次目标：补齐飞书接入最后两个未实测分支——「上传 BOM 文件」与「物料错误检测 + 推送」。
- 实际完成：① 飞书单聊发送 `异常示例_BOM.xlsx` → 机器人正确回执，文件按 `Path().name` 安全落地到 `uploads/`，5229B 与源文件一致；② 不点名文件说"检测一下这个 BOM 有哪些物料错误，并把报告推送到错误报告队列" → 模型正确调 `detect_material_errors(push=true)`，报告落库 `ERR-1F1D3D2841`（PENDING_PUSH），3 行 3 错误、阻断 2 警告 1；③ 顺带验证了 `_last_file` 记忆机制（用户未点名文件时，机器人把上次上传的文件名提示给模型）。
- 改动文件：本文件（登记验证结果）。
- 验证命令与结果：`error_reports` 由 0 → 1（SQLite 直查）；`uploads/` 出现 20:26:24 落地的 5229B 文件；机器人日志含"文件已落地"与"收到文本 open_id=ou_03e7…"两行。
- 遇到的问题：无。
- 遗留问题：无（飞书四条链路：文本对话、BOM→PO 草稿→确认写入、文件上传、错误检测+推送，全部实测通过）。建议重置飞书 App Secret（曾在对话中明文出现）。
- 下一步第一动作：重置 App Secret 并同步 `.env`；或升级飞书回复为消息卡片（当前为纯文本）。
