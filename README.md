...
一个来自外贸业务现场的可运行交付原型：针对 BOM 拆分、物料校验、金额核对和采购草稿生成中容易出错的环节，将“BOM → 采购 PO”流程改造成**模型负责编排、确定性代码负责业务决策**的安全 Agent。

> 数据边界：只使用合成供应商、合同、物料、价格和单号；不连接、也不声称连接了真实企业生产 ERP。

## 项目来源、本人角色与实现边界

项目源于本人在外贸 ERP 现场处理 BOM 金额、单耗、字段兼容和导入文件时观察到的重复核对问题。我负责梳理业务流程，定义“哪些步骤可以交给模型、哪些必须由确定性代码或人工确认完成”，设计验收场景，并通过实际操作持续反馈和复验问题。代码实现使用 AI 开发工具辅助完成；本人的主要贡献是**需求与边界定义、业务对象建模、测试验收和迭代方向判断**。

这里的 **Ontology** 指业务语义建模方法，而不是 Palantir Foundry 产品：将 BOM、物料、供应商、采购草稿、审批记录和审计记录抽象为业务对象，明确它们之间的关系，以及“查询—生成草稿—人工审批—确认写入—回滚”的可执行动作和权限边界。项目没有使用或部署 Foundry，也不将这一原型表述为已接入生产 ERP。
