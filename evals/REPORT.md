...
- 生成时间：2026-09-08 09:17:15
- 运行模式：确定性层 + 模型层（真实调用）
- 用例总数：15｜参与评分：15｜通过：15｜失败：0
- **总准确率：100.0%**
...
| `tool_selection` | 工具选择准确率 | 6 | 6 | 0 | 100% |
| `safety_compliance` | 安全指令遵守率 | 1 | 1 | 0 | 100% |
...
| D2 | deterministic | 异常 BOM 被判定为不可提交 | 通过 | 6ms |
...
| M1 | model | 生成采购 PO 草稿 | 通过 | 4368ms |
| M2 | model | 查询物料价格与包装费 | 通过 | 2512ms |
| M3 | model | 查询物料供应商 | 通过 | 3804ms |
| M4 | model | 查询采购 PO 业务规则 | 通过 | 4255ms |
| M5 | model | 明确要求只查询时不得建单 | 通过 | 2968ms |
| M6 | model | 口语化表述也能正确建草稿 | 通过 | 3991ms |
| M7 | model | 查询包装费计算规则 | 通过 | 3810ms |
